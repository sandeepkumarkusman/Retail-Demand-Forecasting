"""
ML + XYZT blend (50/50) — best of both worlds.
Uses 91-day anchor ML model blended with the XYZT math baseline.
"""
import numpy as np
import pandas as pd
...
def get_xyzt_preds(train_raw, test_raw):
    df = train_raw.copy()
    df['date'] = pd.to_datetime(df['date'])
    df['year'] = df['date'].dt.year
    df['month'] = df['date'].dt.month
    ...
    t['dayofweek'] = t['date'].dt.dayofweek
    grand_avg        = df['sales'].mean()
    store_item_table = df.pivot_table(index='store', columns='item', values='sales', aggfunc='mean')
    month_table      = df.pivot_table(index='month', values='sales', aggfunc='mean') / grand_avg
    dow_table        = df.pivot_table(index='dayofweek', values='sales', aggfunc='mean') / grand_avg
    year_table       = df.pivot_table(index='year', values='sales', aggfunc='mean') / grand_avg
    years = np.arange(2013, 2019)
    avg   = year_table['sales'].values.squeeze()
    w     = np.exp((years - 2018) / 6)
    ...
    def predict_row(row):
        return (store_item_table.at[row['store'], row['item']]
                ...
                * poly(row['year']))
    return t.apply(predict_row, axis=1).values
...
if __name__ == '__main__':
    ...
    print('Computing XYZT baseline...')
    xyzt_preds = get_xyzt_preds(train_raw, test_raw)
    ...
    lgb_preds = np.clip(model.predict(test_df[FEATURES]), 0, None)

    print('Blending 50% ML + 50% XYZT...')
    final_preds = 0.5 * lgb_preds + 0.5 * xyzt_preds

    sub = pd.DataFrame({
        'id':    test_df['id'].fillna(0).astype(int),
        'sales': np.round(final_preds).astype(int),
