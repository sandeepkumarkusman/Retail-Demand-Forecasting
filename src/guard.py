...
def validate_required_files(data_dir: str | Path = "data/raw") -> None:
    ...
    missing_files = [
        filename for filename in REQUIRED_FILES if not (raw_path / filename).is_file()
    ]
    if missing_files:
        raise FileNotFoundError(
            "Missing input file(s) in " f"{raw_path}: {', '.join(missing_files)}"
        )
...
def validate_ml_predictions(predictions: pd.DataFrame, config: dict = None) -> None:
    ...
    if (predictions["sales"] < min_sales).any() or (
        predictions["sales"] > max_sales
    ).any():
        raise ValueError(
            f"Point predictions fall outside acceptable range [{min_sales}, {max_sales}]"
        )
