...
class ModelAccuracyTracker:
    ...
    def detect_drift(
        ...
        if baseline_value is None:
            return {'drift_detected': False, 'reason': f'Metric {metric} not in baseline'}
        ...
        if not recent_values:
            return {'drift_detected': False, 'reason': f'Metric {metric} not in recent runs'}
...
def subset_forecast_by_importance(
    df: pd.DataFrame,
    top_n: int = 100,
    importance_metric: str = 'total_sales'
) -> pd.DataFrame:
    """
