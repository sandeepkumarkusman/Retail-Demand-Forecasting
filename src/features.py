...
def expand_ml_date_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(holiday_dates) > 0:
        ...
        df_merged = pd.merge_asof(df_sorted, holiday_dates, left_on='date', right_on='holiday_date', direction='forward')
...
def add_lag_and_rolling_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.merge(df_store_daily[['store', 'date', 'store_total_sales_lag_91']], on=['store', 'date'], how='left')
    df = df.merge(df_item_daily[['item', 'date', 'item_total_sales_lag_91']], on=['item', 'date'], how='left')
    df.sort_values(['store', 'item', 'date'], inplace=True)
